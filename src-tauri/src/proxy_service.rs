...
use crate::models::ApiProxyRoutingMode;
use crate::models::ApiProxyStatus;
use crate::models::ApiProxyThreadBindingSource;
use crate::models::ApiProxyThreadBindingStatus;
use crate::models::StoredAccount;
use crate::models::UsageSnapshot;
use crate::models::UsageWindow;
use crate::state::ApiProxyCooldownEntry;
use crate::state::ApiProxyRuntimeHandle;
use crate::state::ApiProxyRuntimeState;
use crate::state::ApiProxyThreadBinding;
#[cfg(feature = "desktop")]
...
const PROXY_THREAD_BINDING_TTL_SECS: i64 = 24 * 60 * 60;
const PROXY_THREAD_BINDING_MAX_ITEMS: usize = 256;
const DEFAULT_PROXY_RATE_LIMIT_COOLDOWN_SECS: i64 = 60;
const DEFAULT_PROXY_QUOTA_COOLDOWN_SECS: i64 = 300;
const DEFAULT_PROXY_MODEL_RESTRICTED_COOLDOWN_SECS: i64 = 600;
...
struct ProxyContext {
    ...
    shared: Arc<tokio::sync::Mutex<ApiProxyRuntimeState>>,
}
...
pub(crate) async fn start_api_proxy_with_runtime(
    ...
    let available_accounts = load_proxy_candidates(storage).await?.1;
    ...
    let shared = Arc::new(tokio::sync::Mutex::new(ApiProxyRuntimeState::default()));
...
#[cfg(feature = "desktop")]
pub(crate) async fn set_api_proxy_thread_binding_internal(
    app: &AppHandle,
    state: &AppState,
    session_id: String,
    account_entry_id: Option<String>,
) -> Result<ApiProxyStatus, String> {
    let storage = app_proxy_storage_context(app, state)?;
    set_api_proxy_thread_binding_with_runtime(
        &storage,
        &state.api_proxy,
        session_id,
        account_entry_id,
...
pub(crate) async fn set_api_proxy_thread_binding_with_runtime(
    storage: &ProxyStorageContext,
    runtime_slot: &tokio::sync::Mutex<Option<ApiProxyRuntimeHandle>>,
    session_id: String,
    account_entry_id: Option<String>,
) -> Result<ApiProxyStatus, String> {
    let normalized_session_id = normalize_proxy_session_id(session_id)?;
    let normalized_account_entry_id = account_entry_id.and_then(|value| {
    ...
    let target_candidate = if let Some(account_id) = normalized_account_entry_id.as_deref() {
        let (mode, candidates) = load_proxy_candidates(storage).await?;
        if matches!(mode, ApiProxyRoutingMode::Sequential) {
            return Err("当前为“顺序切换”模式，不能设置线程粘性绑定。".to_string());
        }
        Some(
            candidates
                .into_iter()
                .find(|candidate| candidate.id == account_id)
                .ok_or_else(|| "目标账号不在代理账号池中，或当前不可用于反代。".to_string())?,
        )
    } else {
        None
    };
    ...
    let handle_state = {
        ...
        let Some(handle) = guard.as_ref() else {
            return Err("请先启动 API 反代，再管理线程绑定。".to_string());
        };
        if handle.task.is_finished() {
            return Err("API 反代当前未运行，请重新启动后再管理线程绑定。".to_string());
        }

        let mut shared = handle.shared.lock().await;
        cleanup_runtime_bindings(&mut shared);
        match target_candidate.as_ref() {
            Some(candidate) => upsert_runtime_binding(
                &mut shared,
                &normalized_session_id,
                candidate,
                ApiProxyThreadBindingSource::Manual,
            ),
            None => {
                shared.thread_bindings.remove(&normalized_session_id);
            }
        }
        snapshot_handle_state(handle)
    };

    Ok(status_from_handle_state(handle_state).await)
}
...
async fn chat_completions_handler(
    ...
    let (_candidate, upstream_response) = upstream;
...
async fn send_codex_request_over_candidates(
    ...
    let requested_model_key = extract_requested_model_key(payload);
    let (routing_mode, candidates) = match load_proxy_candidates(&context.storage).await {
        Ok((mode, items)) if !items.is_empty() => (mode, items),
        Ok(_) => {
    ...
    let mut routing = prepare_proxy_routing(context, headers, routing_mode, &candidates).await;
    let (available_candidates, blocked_candidates) = filter_proxy_candidates_by_cooldown(
        context,
        &routing.candidates,
        requested_model_key.as_deref(),
    )
    .await;
    if available_candidates.is_empty() && !blocked_candidates.is_empty() {
        let message = build_cooldown_blocked_error(&blocked_candidates);
        update_proxy_error(context, Some(message.clone())).await;
        return Err(json_error_response(
            StatusCode::SERVICE_UNAVAILABLE,
            &message,
    ...
    routing.candidates = available_candidates;

    let mut attempt_errors = Vec::new();
    let mut retriable_failures = Vec::new();

    for mut candidate in routing.candidates.clone() {
        ...
        loop {
            ...
            if status.is_success() {
                record_proxy_routing_success(context, &routing, &candidate).await;
            ...
            if let Some(failure) = classify_retriable_failure(status, &upstream_body) {
                register_proxy_candidate_cooldown(
                    context,
                    &candidate,
                    requested_model_key.as_deref(),
                    &failure,
...
async fn forward_codex_request_with_candidate(
    ...
    match &candidate.auth {
        ProxyCandidateAuth::Chatgpt { access_token, .. } => {
            forward_chatgpt_codex_request(context, candidate, headers, payload, access_token).await
        }
        ProxyCandidateAuth::Relay {
            base_url,
            api_key,
            model_name,
        } => forward_relay_responses_request(context, payload, base_url, api_key, model_name).await,
    }
}

async fn forward_chatgpt_codex_request(
    context: &ProxyContext,
    candidate: &ProxyCandidate,
    headers: &HeaderMap,
    payload: &Value,
    ...
    let session_id =
        extract_request_session_id(headers).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
...
async fn forward_relay_responses_request(
    context: &ProxyContext,
    payload: &Value,
    base_url: &str,
    api_key: &str,
    ...
    let upstream_payload = payload_with_model(payload, model_name);
    let serialized = serde_json::to_vec(&upstream_payload)
        .map_err(|error| format!("序列化 API 上游请求失败: {error}"))?;
    ...
    context
        ...
        .send()
        .await
        .map_err(|error| format!("请求 API 上游失败 {upstream_url}: {error}"))
}

fn payload_with_model(payload: &Value, model_name: &str) -> Value {
...
#[derive(Clone)]
struct ProxyRoutingSelection {
    mode: ApiProxyRoutingMode,
    session_id: Option<String>,
    existing_binding_source: Option<ApiProxyThreadBindingSource>,
    candidates: Vec<ProxyCandidate>,
}

async fn filter_proxy_candidates_by_cooldown(
    context: &ProxyContext,
    candidates: &[ProxyCandidate],
    requested_model_key: Option<&str>,
) -> (
    Vec<ProxyCandidate>,
    Vec<(ProxyCandidate, ApiProxyCooldownEntry)>,
) {
    let mut shared = context.shared.lock().await;
    cleanup_runtime_cooldowns(&mut shared);

    let mut available = Vec::with_capacity(candidates.len());
    let mut blocked = Vec::new();

    for candidate in candidates.iter().cloned() {
        let cooldown_key = candidate.cooldown_key(requested_model_key);
        if let Some(entry) = shared.model_cooldowns.get(&cooldown_key).cloned() {
            blocked.push((candidate, entry));
        } else {
...
fn build_cooldown_blocked_error(
    blocked_candidates: &[(ProxyCandidate, ApiProxyCooldownEntry)],
) -> String {
    let now = now_unix_seconds();
    let shortest_wait_secs = blocked_candidates
        .iter()
        .map(|(_, cooldown)| (cooldown.expires_at - now).max(0))
        .min()
        .unwrap_or(0);
    let labels = blocked_candidates
        .iter()
        .take(3)
        .map(|(candidate, cooldown)| {
            let remaining = (cooldown.expires_at - now).max(0);
            format!(
                "{}({remaining}s, {})",
                candidate.label,
                truncate_for_error(&cooldown.reason, 24)
            )
        })
        .collect::<Vec<_>>()
        .join("、");
    let suffix = if blocked_candidates.len() > 3 {
        format!(" 等 {} 个账号", blocked_candidates.len())
    } else {
        String::new()
    };

    format!(
        "当前模型的候选账号均处于冷却中，最短约 {} 秒后恢复：{}{}",
        shortest_wait_secs, labels, suffix
    )
}

async fn load_proxy_candidates(
    storage: &ProxyStorageContext,
) -> Result<(ApiProxyRoutingMode, Vec<ProxyCandidate>), String> {
    let _guard = storage.store_lock.lock().await;
    let store = load_store_from_path(&account_store_path_from_data_dir(&storage.data_dir))?;
    let routing_mode = store.settings.api_proxy_routing_mode;
    let account_pool = store.settings.api_proxy_account_pool.clone();

    let mut deduped: HashMap<String, ProxyCandidate> = HashMap::new();
    for candidate in store.accounts.into_iter().filter_map(|account| {
        if !proxy_account_allowed(&account, account_pool.as_deref()) {
            return None;
        }
        account_to_proxy_candidate(account)
    }) {
    ...
    let candidates = order_proxy_candidates(deduped, account_pool.as_deref());
    Ok((routing_mode, candidates))
}

fn order_proxy_candidates(
    deduped: HashMap<String, ProxyCandidate>,
    account_pool: Option<&[String]>,
) -> Vec<ProxyCandidate> {
    if let Some(ordered_ids) = account_pool {
        let mut ordered = Vec::new();
        let mut leftovers = deduped.into_values().collect::<Vec<_>>();
        leftovers.sort_by(compare_proxy_candidates);

        for ordered_id in ordered_ids {
            if let Some(index) = leftovers
                .iter()
                .position(|candidate| candidate.id == *ordered_id)
            {
                ordered.push(leftovers.remove(index));
            }
        }
...
fn extract_request_session_id(headers: &HeaderMap) -> Option<String> {
    headers
        .get("session_id")
        .and_then(|value| value.to_str().ok())
        .map(str::trim)
...
fn extract_requested_model_key(payload: &Value) -> Option<String> {
    payload
        .get("model")
        .and_then(Value::as_str)
        .map(normalize_proxy_model_key)
}

fn normalize_proxy_session_id(session_id: String) -> Result<String, String> {
    let trimmed = session_id.trim();
    if trimmed.is_empty() {
        return Err("线程标识不能为空。".to_string());
    }
    Ok(trimmed.to_string())
}

fn normalize_proxy_model_key(model: &str) -> String {
    let trimmed = model.trim();
    if trimmed.is_empty() {
        return "default".to_string();
    }

    map_client_model_to_upstream(trimmed)
        .unwrap_or_else(|_| trimmed.to_string())
        .to_ascii_lowercase()
}

fn prioritize_candidate_order(
    candidates: &[ProxyCandidate],
    ...
    if let Some(index) = ordered
        .iter()
        .position(|candidate| candidate.id == account_entry_id)
    {
...
fn rotate_candidate_order(
    ...
    let Some(index) = candidates
        .iter()
        .position(|candidate| candidate.id == current_account_id)
    else {
        return candidates.to_vec();
    };

    let mut ordered = Vec::with_capacity(candidates.len());
    ordered.extend_from_slice(&candidates[index..]);
    ordered.extend_from_slice(&candidates[..index]);
    ordered
}

fn cleanup_runtime_bindings(state: &mut ApiProxyRuntimeState) {
    let cutoff = now_unix_seconds() - PROXY_THREAD_BINDING_TTL_SECS;
    state
        .thread_bindings
        .retain(|_, binding| binding.last_seen_at >= cutoff);

    if state.thread_bindings.len() <= PROXY_THREAD_BINDING_MAX_ITEMS {
        return;
    }
    ...
    let mut keys = state
        .thread_bindings
        .values()
        .map(|binding| (binding.session_id.clone(), binding.last_seen_at))
        .collect::<Vec<_>>();
    keys.sort_by(|left, right| right.1.cmp(&left.1));

    for (session_id, _) in keys.into_iter().skip(PROXY_THREAD_BINDING_MAX_ITEMS) {
        state.thread_bindings.remove(&session_id);
    }
}

fn cleanup_runtime_cooldowns(state: &mut ApiProxyRuntimeState) {
    let now = now_unix_seconds();
    state
        .model_cooldowns
        .retain(|_, cooldown| cooldown.expires_at > now);
}

fn upsert_runtime_binding(
    state: &mut ApiProxyRuntimeState,
    session_id: &str,
    candidate: &ProxyCandidate,
    source: ApiProxyThreadBindingSource,
) {
    state.thread_bindings.insert(
        session_id.to_string(),
        ApiProxyThreadBinding {
            session_id: session_id.to_string(),
            account_entry_id: candidate.id.clone(),
            account_label: candidate.label.clone(),
            source,
...
async fn prepare_proxy_routing(
    context: &ProxyContext,
    headers: &HeaderMap,
    mode: ApiProxyRoutingMode,
    candidates: &[ProxyCandidate],
) -> ProxyRoutingSelection {
    let session_id = extract_request_session_id(headers);
    let mut shared = context.shared.lock().await;
    cleanup_runtime_bindings(&mut shared);

    match mode {
        ApiProxyRoutingMode::Sticky => {
            let mut existing_binding_source = None;
            let ordered = if let Some(session_id_value) = session_id.as_deref() {
                if let Some(binding) = shared.thread_bindings.get(session_id_value).cloned() {
                    if candidates
                        .iter()
                        .any(|candidate| candidate.id == binding.account_entry_id)
                    {
                        existing_binding_source = Some(binding.source);
                        prioritize_candidate_order(candidates, &binding.account_entry_id)
                    } else {
                        shared.thread_bindings.remove(session_id_value);
                        candidates.to_vec()
                    }
                } else {
                    candidates.to_vec()
            ...
            ProxyRoutingSelection {
                mode,
                session_id,
                existing_binding_source,
                candidates: ordered,
            }
        }
        ApiProxyRoutingMode::Sequential => ProxyRoutingSelection {
            mode,
            session_id,
            existing_binding_source: None,
            candidates: rotate_candidate_order(candidates, shared.sequence_account_id.as_deref()),
        },
        ApiProxyRoutingMode::Hybrid => {
            let mut existing_binding_source = None;
            if let Some(session_id_value) = session_id.as_deref() {
                if let Some(binding) = shared.thread_bindings.get(session_id_value).cloned() {
                    if candidates
                        ...
                        .any(|candidate| candidate.id == binding.account_entry_id)
                    {
                        if binding.source == ApiProxyThreadBindingSource::Manual {
                            existing_binding_source = Some(ApiProxyThreadBindingSource::Manual);
                            return ProxyRoutingSelection {
                                mode,
                                session_id,
                                existing_binding_source,
                                candidates: prioritize_candidate_order(
                                    candidates,
                                    &binding.account_entry_id,
                                ),
                            };
                        }
                    } else {
            ...
            ProxyRoutingSelection {
                ...
                session_id,
                existing_binding_source,
                candidates: rotate_candidate_order(
                    candidates,
                    shared.sequence_account_id.as_deref(),
                ),
            }
        }
    }
...
async fn record_proxy_routing_success(
    context: &ProxyContext,
    routing: &ProxyRoutingSelection,
    candidate: &ProxyCandidate,
) {
    let mut state = context.shared.lock().await;
    cleanup_runtime_bindings(&mut state);
    state.active_account_key = Some(candidate.account_key.clone());
    state.active_account_id = Some(candidate.account_id.clone());
    state.active_account_label = Some(candidate.label.clone());

    match routing.mode {
        ApiProxyRoutingMode::Sticky => {
            if let Some(session_id) = routing.session_id.as_deref() {
                let source = routing
                    .existing_binding_source
                    .unwrap_or(ApiProxyThreadBindingSource::Auto);
                upsert_runtime_binding(&mut state, session_id, candidate, source);
            }
        }
        ApiProxyRoutingMode::Sequential => {
            state.sequence_account_id = Some(candidate.id.clone());
            state.sequence_account_label = Some(candidate.label.clone());
            if let Some(session_id) = routing.session_id.as_deref() {
                upsert_runtime_binding(
                    &mut state,
                    session_id,
                    candidate,
                    ApiProxyThreadBindingSource::Auto,
                );
            }
        }
        ApiProxyRoutingMode::Hybrid => {
            if matches!(
                routing.existing_binding_source,
                Some(ApiProxyThreadBindingSource::Manual)
            ) {
                if let Some(session_id) = routing.session_id.as_deref() {
                    upsert_runtime_binding(
                        &mut state,
    ...
    match account.source_kind {
        AccountSourceKind::Chatgpt => {
            ...
            Some(ProxyCandidate {
                ...
                auth: ProxyCandidateAuth::Chatgpt {
        ...
        AccountSourceKind::Relay => {
            ...
            let model_name = account.model_name?;
            if base_url.is_empty() || api_key.trim().is_empty() || model_name.trim().is_empty() {
                return None;
            }
            Some(ProxyCandidate {
                id: account.id,
                ...
                auth: ProxyCandidateAuth::Relay {
                    base_url,
                    api_key,
                    model_name,
                },
                variant_key,
                plan_type: Some(account.plan_type.unwrap_or_else(|| "api".to_string())),
                usage: account.usage,
...
fn proxy_account_allowed(account: &StoredAccount, account_pool: Option<&[String]>) -> bool {
    match account_pool {
        Some(ids) => ids.iter().any(|id| id == &account.id),
        None => matches!(account.source_kind, AccountSourceKind::Chatgpt),
    }
}
...
fn compare_proxy_candidates(left: &ProxyCandidate, right: &ProxyCandidate) -> Ordering {
    ...
    match left.source_rank().cmp(&right.source_rank()) {
        Ordering::Equal => {}
        ordering => return ordering,
    }
...
impl ProxyCandidate {
    fn is_chatgpt(&self) -> bool {
        matches!(&self.auth, ProxyCandidateAuth::Chatgpt { .. })
    }

    fn source_rank(&self) -> i32 {
        match &self.auth {
            ProxyCandidateAuth::Chatgpt { .. } => 0,
            ProxyCandidateAuth::Relay { .. } => 1,
        }
    }

    fn cooldown_model_key(&self, requested_model_key: Option<&str>) -> String {
        match &self.auth {
            ProxyCandidateAuth::Chatgpt { .. } => {
                requested_model_key.unwrap_or("default").to_string()
            }
            ProxyCandidateAuth::Relay { model_name, .. } => normalize_proxy_model_key(model_name),
        }
    }

    fn cooldown_key(&self, requested_model_key: Option<&str>) -> String {
        proxy_cooldown_key(
            &self.account_key,
            &self.cooldown_model_key(requested_model_key),
        )
    }
}

fn proxy_cooldown_key(account_key: &str, model_key: &str) -> String {
    format!("{account_key}::{model_key}")
}

fn retry_after_secs_from_headers(headers: &HeaderMap) -> Option<i64> {
    headers
        .get("retry-after")
        .and_then(|value| value.to_str().ok())
        .map(str::trim)
...
fn cooldown_secs_for_failure(
    category: RetryFailureCategory,
    retry_after_secs: Option<i64>,
) -> Option<i64> {
    match category {
        RetryFailureCategory::RateLimited => Some(
            retry_after_secs
                .unwrap_or(DEFAULT_PROXY_RATE_LIMIT_COOLDOWN_SECS)
                .clamp(5, 3_600),
        ),
        RetryFailureCategory::QuotaExceeded => Some(DEFAULT_PROXY_QUOTA_COOLDOWN_SECS),
        RetryFailureCategory::ModelRestricted => Some(DEFAULT_PROXY_MODEL_RESTRICTED_COOLDOWN_SECS),
        RetryFailureCategory::Authentication | RetryFailureCategory::Permission => None,
    }
}

async fn register_proxy_candidate_cooldown(
    context: &ProxyContext,
    candidate: &ProxyCandidate,
    requested_model_key: Option<&str>,
    ...
    let Some(cooldown_secs) = cooldown_secs_for_failure(
        failure.category,
        retry_after_secs_from_headers(upstream_headers),
    ) else {
        return;
    };

    let model_key = candidate.cooldown_model_key(requested_model_key);
    let cooldown_key = proxy_cooldown_key(&candidate.account_key, &model_key);
    let mut shared = context.shared.lock().await;
    cleanup_runtime_cooldowns(&mut shared);
    shared.model_cooldowns.insert(
        cooldown_key,
        ApiProxyCooldownEntry {
            account_key: candidate.account_key.clone(),
            model_key,
            reason: failure.detail.clone(),
            expires_at: now_unix_seconds() + cooldown_secs,
        },
    );
}
...
async fn refresh_proxy_candidate_auth(
    ...
    let auth_json = match &candidate.auth {
        ProxyCandidateAuth::Chatgpt { auth_json, .. } => auth_json,
        ProxyCandidateAuth::Relay { .. } => {
            return Err("API 条目不支持 OAuth 登录态刷新。".to_string());
        }
    };
    let refreshed_auth_json =
        match refresh_chatgpt_auth_tokens_serialized(auth_json, &storage.auth_refresh_lock).await {
...
async fn status_from_handle_state(handle: ApiProxyHandleState) -> ApiProxyStatus {
    let snapshot = handle.shared.lock().await.clone();
    let thread_bindings = runtime_thread_binding_statuses(&snapshot);
    if handle.task_finished {
        ApiProxyStatus {
            ...
            sequence_account_id: snapshot.sequence_account_id,
            sequence_account_label: snapshot.sequence_account_label,
            thread_bindings,
...
fn runtime_thread_binding_statuses(
    snapshot: &ApiProxyRuntimeState,
) -> Vec<ApiProxyThreadBindingStatus> {
    let mut items = snapshot
        .thread_bindings
        .values()
        .map(|binding| ApiProxyThreadBindingStatus {
            session_id: binding.session_id.clone(),
            account_entry_id: binding.account_entry_id.clone(),
...
fn stopped_status(api_key: Option<String>, last_error: Option<String>) -> ApiProxyStatus {
    ApiProxyStatus {
        ...
        sequence_account_id: None,
        sequence_account_label: None,
