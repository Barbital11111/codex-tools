...
async fn update_api_account(
...

    {
        ...
        if let Some(handle) = api_proxy.as_ref() {
            let mut snapshot = handle.shared.lock().await;
            if snapshot.active_account_key.as_deref() == Some(account_key.as_str()) {
                snapshot.active_account_label = Some(summary.label.clone());
            }
        }
    }
...
#[tauri::command]
async fn set_api_proxy_thread_binding(
    app: AppHandle,
    state: State<'_, AppState>,
    session_id: String,
    account_entry_id: Option<String>,
) -> Result<ApiProxyStatus, String> {
    proxy_service::set_api_proxy_thread_binding_internal(
        &app,
        state.inner(),
        session_id,
        account_entry_id,
