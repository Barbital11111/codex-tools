...
pub(crate) async fn delete_account_internal(
    ...
    if let Some(pool) = store.settings.api_proxy_account_pool.as_mut() {
        pool.retain(|account_id| account_id != id);
    }
    if store.settings.api_proxy_fixed_account_id.as_deref() == Some(id) {
...
pub(crate) async fn update_api_account_internal(
    ...
    let updated_account = {
        ...
        let resolved_api_key = match input
            .api_key
            .as_deref()
            .map(str::trim)
            .filter(|value| !value.is_empty())
        {
            Some(api_key) => profile_files::normalize_relay_api_key(api_key)?,
            None => account
                .api_key
                .clone()
                .ok_or_else(|| "当前 API 账号缺少可复用的 Key，请重新填写".to_string())?,
        };

        let (provider_id, provider_name) =
            infer_provider_metadata_from_base_url(Some(resolved_base_url.as_str()));

        account.label = resolved_label;
        account.auth_json = profile_files::build_api_auth_json(&resolved_api_key);
        account.api_base_url = Some(resolved_base_url);
        account.api_key = Some(resolved_api_key);
...
pub(crate) async fn refresh_all_usage_internal(
    ...
    refresh_usage_internal(app, state, force_auth_refresh).await
}
