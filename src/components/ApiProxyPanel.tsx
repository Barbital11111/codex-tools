...
import { sortAccountsByRemaining } from "../utils/accountRanking";
import {
  displayAccountLabel,
  displayAccountLabelFallback,
  displayModelName,
  displayRelayEndpoint,
  isRelayAccountReference,
} from "../utils/privacy";
...
type ThreadRouteFilter = "all" | "manual" | "auto";
...
type ApiProxyPanelProps = {
  ...
  savedPort: number;
  routingMode: ApiProxyRoutingMode;
  ...
  onUpdateProxySettings: (patch: Partial<AppSettings>) => void;
  onSetApiProxyThreadBinding: (sessionId: string, accountEntryId: string | null) => Promise<void> | void;
...
function formatProxyAccountMeta(
  account: AccountSummary,
  proxyCopy: MessageCatalog["apiProxy"],
  hideAccountDetails: boolean,
) {
  ...
  const fiveHour = remainingPercent(account.usage?.fiveHour ?? null);
  const oneWeek = remainingPercent(account.usage?.oneWeek ?? null);
  if (fiveHour === null && oneWeek === null) {
    return proxyCopy.poolUsageFallback;
  }
  return `5h ${percent(fiveHour)} · 1w ${percent(oneWeek)}`;
}
...
export function ApiProxyPanel({
  ...
  const [proxyAccountPoolDraft, setProxyAccountPoolDraft] = useState<string[] | null>(accountPool);
  const [proxyRoutingModeDraft, setProxyRoutingModeDraft] =
    useState<ApiProxyRoutingMode>(routingMode);
  const [threadBindingPendingSessionId, setThreadBindingPendingSessionId] = useState<string | null>(
    null,
  );
  const [threadRouteFilter, setThreadRouteFilter] = useState<ThreadRouteFilter>("all");
  const [threadRouteSearch, setThreadRouteSearch] = useState("");
  const [proxyImportPoolId, setProxyImportPoolId] = useState("");
  const proxyAccountPoolDraftRef = useRef<string[] | null>(accountPool);
  const proxyPoolLocalEditTimerRef = useRef<number | null>(null);
  const proxyPoolHasLocalEditRef = useRef(false);
  ...
  useEffect(() => {
    if (proxyPoolHasLocalEditRef.current) {
      return;
    }
    proxyAccountPoolDraftRef.current = accountPool;
    setProxyAccountPoolDraft(accountPool);
  }, [accountPool]);

  useEffect(
    () => () => {
      if (proxyPoolLocalEditTimerRef.current !== null) {
        window.clearTimeout(proxyPoolLocalEditTimerRef.current);
      }
    },
    [],
  );
  ...
  useEffect(() => {
    setProxyRoutingModeDraft(routingMode);
  }, [routingMode]);

  useEffect(() => {
    if (accountPools.length === 0) {
      setProxyImportPoolId("");
      return;
    }
    if (proxyImportPoolId && accountPools.some((pool) => pool.id === proxyImportPoolId)) {
      return;
    }
    setProxyImportPoolId(accountPools[0]?.id ?? "");
  }, [accountPools, proxyImportPoolId]);
  ...
  const activeProxyAccount = status.activeAccountId
    ? accountById.get(status.activeAccountId) ?? null
    : null;
  const accountIdsByKey = useMemo(() => {
    ...
    for (const account of accounts) {
      ...
      if (existing) {
        ...
        groups.set(account.accountKey, [account.id]);
      }
    }
    return groups;
  }, [accounts]);
  const defaultProxyAccountIds = accounts
    .filter((account) => account.sourceKind === "chatgpt")
    .map((account) => account.id);
  const selectedProxyAccountIds = proxyAccountPoolDraft ?? defaultProxyAccountIds;
  const selectedProxyAccountIdSet = new Set(selectedProxyAccountIds);
  const selectedProxyAccounts = selectedProxyAccountIds
    .map((id) => accountById.get(id))
    .filter((account): account is AccountSummary => Boolean(account));
  const unselectedProxyAccounts = sortAccountsByRemaining(
    accounts.filter((account) => !selectedProxyAccountIdSet.has(account.id)),
  );
  const orderedProxyAccounts = [...selectedProxyAccounts, ...unselectedProxyAccounts];
  const importableAccountPools = useMemo(
    () =>
      accountPools.map((pool) => ({
        ...pool,
        resolvedIds: Array.from(
          new Set(pool.accountKeys.flatMap((accountKey) => accountIdsByKey.get(accountKey) ?? [])),
        ),
      })),
    [accountIdsByKey, accountPools],
  );
  const selectedImportPool =
    importableAccountPools.find((pool) => pool.id === proxyImportPoolId) ?? null;
  const visibleThreadBindings = useMemo(() => {
    const keyword = threadRouteSearch.trim().toLowerCase();

    return status.threadBindings.filter((binding) => {
      if (threadRouteFilter === "manual" && binding.source !== "manual") {
        return false;
      }
      if (threadRouteFilter === "auto" && binding.source !== "auto") {
        return false;
      }
      if (!keyword) {
        return true;
      ...
      return (
        binding.sessionId.toLowerCase().includes(keyword) ||
        binding.accountLabel.toLowerCase().includes(keyword) ||
        binding.accountEntryId.toLowerCase().includes(keyword)
      );
    });
  }, [status.threadBindings, threadRouteFilter, threadRouteSearch]);
  const hasThreadRouteFilter = threadRouteSearch.trim() !== "" || threadRouteFilter !== "all";
  const canManageThreadBindings = proxyRoutingModeDraft !== "sequential";
  const sequenceCursorAccount = status.sequenceAccountId
    ? accountById.get(status.sequenceAccountId) ?? null
    : null;
  const activeProxyAccountLabel =
    status.activeAccountLabel || activeProxyAccount
      ? displayAccountLabelFallback(
          activeProxyAccount,
          status.activeAccountLabel,
          hideAccountDetails,
          isRelayAccountReference(status.activeAccountId) ||
            isRelayAccountReference(status.activeAccountKey),
        )
      : proxyCopy.activeAccountEmptyTitle;
  const sequenceCursorLabel =
    status.sequenceAccountLabel || status.sequenceAccountId || sequenceCursorAccount
      ? displayAccountLabelFallback(
          sequenceCursorAccount,
          status.sequenceAccountLabel ?? status.sequenceAccountId,
          hideAccountDetails,
          isRelayAccountReference(status.sequenceAccountId),
        )
      : proxyCopy.sequenceCursorEmpty;
  const modeDescription =
    proxyRoutingModeDraft === "sticky"
      ? proxyCopy.poolModeStickyDescription
      : proxyRoutingModeDraft === "sequential"
        ? proxyCopy.poolModeSequentialDescription
        : proxyCopy.poolModeHybridDescription;
  const threadRoutesDescription =
    proxyRoutingModeDraft === "sticky"
      ? proxyCopy.threadRoutesDescriptionSticky
      : proxyRoutingModeDraft === "sequential"
        ? proxyCopy.threadRoutesDescriptionSequential
        : proxyCopy.threadRoutesDescriptionHybrid;
  const threadManualResetLabel =
    proxyRoutingModeDraft === "sticky"
      ? proxyCopy.threadRestoreAuto
      : proxyCopy.threadRestoreSequence;
  ...
  const markProxyPoolLocalEdit = () => {
    proxyPoolHasLocalEditRef.current = true;
    if (proxyPoolLocalEditTimerRef.current !== null) {
      window.clearTimeout(proxyPoolLocalEditTimerRef.current);
    }
    proxyPoolLocalEditTimerRef.current = window.setTimeout(() => {
      proxyPoolHasLocalEditRef.current = false;
      proxyPoolLocalEditTimerRef.current = null;
    }, 1200);
  };

  const updateProxyPool = (nextIds: string[]) => {
    const uniqueIds = nextIds.filter((id, index, ids) => id && ids.indexOf(id) === index);
    markProxyPoolLocalEdit();
    proxyAccountPoolDraftRef.current = uniqueIds;
    setProxyAccountPoolDraft(uniqueIds);
    onUpdateProxySettings({ apiProxyAccountPool: uniqueIds });
  };

  const toggleProxyAccount = (account: AccountSummary) => {
    const currentIds = proxyAccountPoolDraftRef.current ?? defaultProxyAccountIds;
    const nextIds = currentIds.includes(account.id)
      ? currentIds.filter((id) => id !== account.id)
      : [...currentIds, account.id];
    updateProxyPool(nextIds);
  };

  const moveProxyAccountPriority = (accountId: string, offset: -1 | 1) => {
    const currentIds = proxyAccountPoolDraftRef.current ?? defaultProxyAccountIds;
    const currentIndex = currentIds.indexOf(accountId);
    ...
    const [accountIdToMove] = nextIds.splice(currentIndex, 1);
    nextIds.splice(nextIndex, 0, accountIdToMove);
    updateProxyPool(nextIds);
  };

  const autoSortProxyPool = () => {
    if (selectedProxyAccounts.length <= 1) {
      return;
    }
    updateProxyPool(sortAccountsByRemaining(selectedProxyAccounts).map((account) => account.id));
  };

  const importProxyPoolFromGroup = (replaceExisting: boolean) => {
    if (!selectedImportPool || selectedImportPool.resolvedIds.length === 0) {
      return;
    }
    const nextIds = replaceExisting
      ? selectedImportPool.resolvedIds
      : Array.from(new Set([...selectedProxyAccountIds, ...selectedImportPool.resolvedIds]));
    updateProxyPool(nextIds);
  };

  const updateProxyRoutingMode = (mode: ApiProxyRoutingMode) => {
    setProxyRoutingModeDraft(mode);
    onUpdateProxySettings({ apiProxyRoutingMode: mode });
  };

  const handleThreadBindingChange = async (sessionId: string, accountEntryId: string | null) => {
    if (!canManageThreadBindings || threadBindingPendingSessionId === sessionId) {
      return;
    }

    setThreadBindingPendingSessionId(sessionId);
    try {
      await onSetApiProxyThreadBinding(sessionId, accountEntryId);
    } catch {
      // Notice 已由 controller 统一处理。
    } finally {
      setThreadBindingPendingSessionId((current) => (current === sessionId ? null : current));
    }
  };
        ...
        <section className="proxySectionCard proxySectionCardPrimary">
          <div className="proxyHeaderStats">
            ...
            <span className="proxyHeaderStat">
              ...
              <strong>{selectedProxyAccounts.length}/{accountCount}</strong>
            </span>
          </div>
            ...
            <div className="proxyControlActions">
              ...
              {status.running ? (
                ...
                <button
                  ...
                  disabled={busy || selectedProxyAccounts.length === 0 || effectivePort === null}
                >
          ...
          <div className="proxyDetailGrid">
            ...
            <article className="proxyDetailCard">
              ...
              <strong>{activeProxyAccountLabel}</strong>
              <p>
                {status.activeAccountId
                  ? hideAccountDetails
                    ? "账号信息已隐藏"
                    : status.activeAccountId
                  : proxyCopy.activeAccountEmptyDescription}
              </p>
            </article>

            {proxyRoutingModeDraft !== "sticky" ? (
              <article className="proxyDetailCard">
                <span className="proxyLabel">{proxyCopy.sequenceCursorLabel}</span>
                <strong>{sequenceCursorLabel}</strong>
                <p>{modeDescription}</p>
              </article>
            ) : null}
        ...
        <section className="proxySectionCard">
          <div className="proxySectionHeader">
            <div className="remoteSectionHeading">
              <h3>{proxyCopy.poolTitle}</h3>
              <p>{proxyCopy.poolDescription}</p>
            </div>
            <span className="proxyPoolCount">
              {proxyCopy.poolCountLabel} {selectedProxyAccounts.length}/{accounts.length}
            </span>
          </div>

          <div className="proxyPoolControls">
            <div
              className="proxyRouteModeGroup"
              role="radiogroup"
              aria-label={proxyCopy.poolModeLabel}
            >
              {([
                ["sticky", proxyCopy.poolModeSticky],
                ["sequential", proxyCopy.poolModeSequential],
                ["hybrid", proxyCopy.poolModeHybrid],
              ] as const).map(([mode, label]) => (
                <button
                  key={mode}
                  type="button"
                  className={proxyRoutingModeDraft === mode ? "primary" : "ghost"}
                  aria-pressed={proxyRoutingModeDraft === mode}
                  onClick={() => updateProxyRoutingMode(mode)}
                  disabled={savingSettings}
                >
                  {label}
                </button>
            ...
            <div className="proxyPoolActions">
              {proxyRoutingModeDraft !== "sticky" ? (
                <span className="proxyPoolSequenceBadge">
                  <span>{proxyCopy.sequenceCursorLabel}</span>
                  <strong>{sequenceCursorLabel}</strong>
                </span>
              ) : null}
              <button
                type="button"
                className="ghost"
                onClick={autoSortProxyPool}
                disabled={savingSettings || selectedProxyAccounts.length <= 1}
              >
                {proxyCopy.poolPriorityAuto}
              </button>
            </div>
          </div>

          <p className="proxyModeHint">{modeDescription}</p>

          {accountPools.length > 0 ? (
            <div className="proxyPoolImportBar">
              <label className="proxyPoolImportField">
                <span>{proxyCopy.poolImportLabel}</span>
                <select
                  className="proxySelect"
                  value={proxyImportPoolId}
                  onChange={(event) => setProxyImportPoolId(event.target.value)}
                  disabled={savingSettings}
                >
                  {importableAccountPools.map((pool) => (
                    <option key={pool.id} value={pool.id}>
                      {pool.name} ({pool.accountKeys.length})
                    </option>
                  ))}
                </select>
              </label>
              <button
                type="button"
                className="ghost"
                onClick={() => importProxyPoolFromGroup(false)}
                disabled={savingSettings || !selectedImportPool || selectedImportPool.resolvedIds.length === 0}
              >
                {proxyCopy.poolImportAppend}
              </button>
              <button
                type="button"
                className="ghost"
                onClick={() => importProxyPoolFromGroup(true)}
                disabled={savingSettings || !selectedImportPool || selectedImportPool.resolvedIds.length === 0}
              >
                {proxyCopy.poolImportReplace}
              </button>
              <span className="proxyPoolImportHint">
                {selectedImportPool
                  ? `${selectedImportPool.accountKeys.length} / ${selectedImportPool.resolvedIds.length} · ${proxyCopy.poolImportHint}`
                  : proxyCopy.poolImportPlaceholder}
              </span>
            </div>
          ) : (
            <div className="proxyPoolImportEmpty">{proxyCopy.poolImportEmpty}</div>
          )}

          {accounts.length === 0 ? (
            <div className="proxyPoolEmpty">
              <strong>{proxyCopy.poolEmptyTitle}</strong>
              <p>{proxyCopy.poolEmptyDescription}</p>
            </div>
          ) : (
            <div className="proxyPoolList">
              {orderedProxyAccounts.map((account) => {
                const selected = selectedProxyAccountIdSet.has(account.id);
                const selectedIndex = selected ? selectedProxyAccountIds.indexOf(account.id) : -1;
                const isRelay = account.sourceKind === "relay";
                const accountType = isRelay
                  ? proxyCopy.poolAccountTypeRelay
                  : proxyCopy.poolAccountTypeChatgpt;
                const planLabel = isRelay
                  ? proxyCopy.poolRelayResponses
                  : formatPlan(
                      account.planType || account.usage?.planType,
                      copy.accountCard.planLabels,
                    );
                const identityLabels = [
                  account.isCurrent ? proxyCopy.poolAccountCurrent : null,
                  account.authRefreshBlocked ? proxyCopy.poolAccountBlocked : null,
                  account.profileIntegrityError ? proxyCopy.poolAccountIncomplete : null,
                ].filter((value): value is string => Boolean(value));

                return (
                  <label
                    key={account.id}
                    className={`proxyPoolItem${selected ? " isSelected" : ""}${
                      account.profileIntegrityError ? " isIncomplete" : ""
                    }`}
                  >
                    <input
                      type="checkbox"
                      checked={selected}
                      disabled={savingSettings}
                      onChange={() => toggleProxyAccount(account)}
                    />
                    <span className="proxyPoolItemBody">
                      <span className="proxyPoolItemTop">
                        <strong>{displayAccountLabel(account, hideAccountDetails)}</strong>
                        {identityLabels.length > 0 ? (
                          <span className="proxyPoolIdentityBadges">
                      ...
                      <span className="proxyPoolItemMeta">
                        {formatProxyAccountMeta(account, proxyCopy, hideAccountDetails)}
                      </span>
                    </span>
                    <span className="proxyPoolMetaRail">
                      <span className="proxyPoolStatusStack">
                        {selected ? (
                          <span>
                            {proxyCopy.poolPriorityLabel} #{selectedIndex + 1}
                          </span>
                        ) : null}
                        <span>{accountType}</span>
                        <span>{planLabel}</span>
                        <span>{selected ? proxyCopy.poolSelected : proxyCopy.poolNotSelected}</span>
                      </span>

                      {selected ? (
                        <span className="proxyPoolOrderControls">
                          <button
                            type="button"
                            className="ghost proxyOrderButton"
                            aria-label={proxyCopy.poolMoveUp}
                            disabled={savingSettings || selectedIndex <= 0}
                            onClick={(event) => {
                              event.preventDefault();
                              event.stopPropagation();
                              moveProxyAccountPriority(account.id, -1);
                            }}
                          >
                          ...
                          <button
                            type="button"
                            className="ghost proxyOrderButton"
                            aria-label={proxyCopy.poolMoveDown}
                            disabled={
                              savingSettings || selectedIndex >= selectedProxyAccountIds.length - 1
                            }
                            onClick={(event) => {
                              event.preventDefault();
                              event.stopPropagation();
          ...
          <div className="proxySubsection">
            <div className="proxySubsectionHeader">
              <div className="remoteSectionHeading">
                <h4>{proxyCopy.threadRoutesTitle}</h4>
                <p>{threadRoutesDescription}</p>
              </div>
              <span className="proxyPoolCount">
                {proxyCopy.threadRoutesCountLabel} {visibleThreadBindings.length}/
                {status.threadBindings.length}
              </span>
            </div>

            <div className="proxyThreadToolbar">
              <div
                className="proxyRouteModeGroup"
                role="radiogroup"
                aria-label={proxyCopy.threadFilterLabel}
              >
                {([
                  ["all", proxyCopy.threadFilterAll],
                  ["manual", proxyCopy.threadFilterManual],
                  ["auto", proxyCopy.threadFilterAuto],
                ] as const).map(([value, label]) => (
                  <button
                    key={value}
                    type="button"
                    className={threadRouteFilter === value ? "primary" : "ghost"}
                    aria-pressed={threadRouteFilter === value}
                    onClick={() => setThreadRouteFilter(value)}
                  >
                    {label}
                  </button>
                ))}
              ...
              <label className="proxyThreadSearchField">
                <span>{proxyCopy.threadSearchLabel}</span>
                <input
                  className="proxyPortInput proxyThreadSearchInput"
                  value={threadRouteSearch}
                  onChange={(event) => setThreadRouteSearch(event.target.value)}
                  placeholder={proxyCopy.threadSearchPlaceholder}
                />
              </label>
            </div>

            {status.threadBindings.length === 0 ? (
              <div className="proxyPoolEmpty">
                <strong>{proxyCopy.threadRoutesEmptyTitle}</strong>
                <p>{proxyCopy.threadRoutesEmptyDescription}</p>
              </div>
            ) : visibleThreadBindings.length === 0 ? (
              <div className="proxyPoolEmpty">
                <strong>{proxyCopy.threadRoutesNoMatchTitle}</strong>
                <p>
                  {hasThreadRouteFilter
                    ? proxyCopy.threadRoutesNoMatchDescription
                    : proxyCopy.threadRoutesEmptyDescription}
                </p>
              </div>
            ) : (
              <div className="proxyThreadList">
                {visibleThreadBindings.map((binding) => {
                  const bindingPending = threadBindingPendingSessionId === binding.sessionId;
                  const bindingSelectValue = selectedProxyAccountIdSet.has(binding.accountEntryId)
                    ? binding.accountEntryId
                    : "";
                  const bindingAccount = accountById.get(binding.accountEntryId) ?? null;
                  const bindingAccountLabel = displayAccountLabelFallback(
                    bindingAccount,
                    binding.accountLabel,
                    hideAccountDetails,
                    isRelayAccountReference(binding.accountEntryId),
                  );

                  return (
                    <article className="proxyThreadItem" key={binding.sessionId}>
                      <div className="proxyThreadBody">
                        <div className="proxyThreadHeader">
                          <strong>{formatShortSessionId(binding.sessionId)}</strong>
                          <span className="proxyPoolIdentityBadges">
                            <span>
                              {binding.source === "manual"
                                ? proxyCopy.threadSourceManual
                                : proxyCopy.threadSourceAuto}
                            </span>
                          </span>
                        </div>

                        <div className="proxyThreadMeta">
                          <div>
                            <span>{proxyCopy.threadSessionLabel}</span>
                            <code>{binding.sessionId}</code>
                          </div>
                          <div>
                            <span>{proxyCopy.threadAccountLabel}</span>
                            <strong>{bindingAccountLabel}</strong>
                          </div>
                          <div>
                            <span>{proxyCopy.threadLastSeenLabel}</span>
                            <strong>
                              {formatRemoteHistoryTime(locale, binding.lastSeenAt * 1000)}
                            </strong>
                          </div>
                        </div>
                      </div>

                      <div className="proxyThreadActions">
                        <label className="proxyThreadSelectField">
                          <span>{proxyCopy.threadAccountLabel}</span>
                          <select
                            className="proxySelect"
                            value={bindingSelectValue}
                            disabled={
                              !canManageThreadBindings ||
                              selectedProxyAccounts.length === 0 ||
                              bindingPending
                            }
                            onChange={(event) => {
                              void handleThreadBindingChange(
                                binding.sessionId,
                                event.target.value || null,
                              );
                            }}
                          >
                            {!selectedProxyAccountIdSet.has(binding.accountEntryId) ? (
                              <option value="">{proxyCopy.threadMissingAccount}</option>
                            ) : null}
                            {selectedProxyAccounts.map((account) => (
                              <option key={account.id} value={account.id}>
                                {displayAccountLabel(account, hideAccountDetails)}
                              </option>
                            ))}
                          </select>
                        </label>
                        ...
                        {canManageThreadBindings ? (
                          binding.source === "manual" ? (
                            <button
                              ...
                              onClick={() => {
                                void handleThreadBindingChange(binding.sessionId, null);
                              }}
                            >
                              {bindingPending
                                ? proxyCopy.threadApplying
                                : threadManualResetLabel}
                            </button>
                          ) : (
                            <span className="proxyThreadHint">
                              {bindingPending
                                ? proxyCopy.threadApplying
                                : proxyCopy.threadAutoHint}
                            </span>
                          )
                        ) : (
                          <span className="proxyThreadHint">
                            {proxyCopy.threadBindingUnavailable}
                          </span>
                        )}
                      </div>
                    </article>
