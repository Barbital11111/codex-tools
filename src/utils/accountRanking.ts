...
function compareSourceKind(left: AccountSummary, right: AccountSummary): number {
  if (left.sourceKind === right.sourceKind) {
    return 0;
  }
  return left.sourceKind === "chatgpt" ? -1 : 1;
}

export function compareAccountsForDisplay(a: AccountSummary, b: AccountSummary): number {
  const sourceKindDiff = compareSourceKind(a, b);
  if (sourceKindDiff !== 0) {
  ...
  return a.accountKey.localeCompare(b.accountKey, "zh-Hans-CN");
}
