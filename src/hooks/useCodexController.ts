...
const DEFAULT_SETTINGS: AppSettings = {
  ...
  apiProxyAccountPool: null,
  apiProxyFixedAccountId: null,
  accountPools: [],
  remoteServers: [],
...
const DEFAULT_API_PROXY_STATUS: ApiProxyStatus = {
  ...
  sequenceAccountId: null,
  sequenceAccountLabel: null,
  threadBindings: [],
  lastError: null,
};
...
const HIDE_ACCOUNT_DETAILS_STORAGE_KEY = "codex-tools:hide-account-details";
...
export function useCodexController() {
  ...
  const onSetApiProxyThreadBinding = useCallback(
    async (sessionId: string, accountEntryId: string | null) => {
      try {
        const status = await invoke<ApiProxyStatus>("set_api_proxy_thread_binding", {
          sessionId,
          accountEntryId,
        });
        setApiProxyStatus(localizeApiProxyStatus(status));
        ...
        setNotice({
          type: "error",
          message: copy.notices.proxyThreadBindingFailed(localizeError(String(error))),
        });
        throw error;
      }
    },
    [copy.notices, localizeApiProxyStatus, localizeError],
  );
      ...
      try {
        ...
        setApiProxyStatus((prev) =>
          prev.activeAccountKey === account.accountKey
            ? {
                ...prev,
                activeAccountLabel: updated.label,
              }
            : prev,
        );
        setNotice({
  ...
  }, [copy.notices, localizeError, noticeAccountLabel, pendingDeleteId]);
