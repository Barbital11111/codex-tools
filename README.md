...
应用内检查更新依赖 GitHub Release 中的 `latest.json` 与安装包签名文件 `.sig`。

- 若只上传 `.exe/.msi`，但没有 `latest.json`，应用内会提示无法获取有效的 release JSON
- 若丢失当前版本对应的 updater 私钥，则这批已安装用户无法继续自动升级，只能手动覆盖安装一次后迁移到新密钥

详细说明见：[docs/updater-release.md](docs/updater-release.md)
